...
server <- function(input, output) {
  output$coolplot <- renderPlot({
    ...
    H0H1<-ifelse(dbinom(x,n,H0)>dbinom(x,n,H1),"The null hypothesis is more likely than the alternative hypothesis","The alternative hypothesis is more likely than the null hypothesis") 
    ...
    #TES
    binom.p<-binom.test(n=n, x=x, H1, conf.level = .9, alternative = "greater")$p.value
    ...
    output$text4 <- renderText({ 
      paste("The Test of Excessive Significance  (Ioannidis & Trikalinos, 2007) would yield a p-value of ", round(binom.p, 3),". A significant p-value (using an alpha of 0.10) indicates the observed results can be considered surprising, assuming there is no bias in the literature.")
    })
